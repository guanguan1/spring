...
public class ThreadPool {
    ...
    public void testCommon(ThreadPoolExecutor threadPoolExecutor) throws InterruptedException {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            threadPoolExecutor.submit(new Runnable() {
                ...
                public void run() {
                    try {
                        System.out.println("开始执行s:" + n);
                        Thread.sleep(3000L);
                        System.out.println("执行结束end：" + n);
    ...
    /**
     * 1、线程池信息：核心线程数量5，最大数量10，无界队列，超出核心线程数量的线程存活时间：5s，指定拒绝策略
     */
    ...
    private void threadPoolExecutorTest7() throws InterruptedException {
        ...
        submitThread(threadPoolExecutor);
        Thread.sleep(1000L);
    ...
    private void submitThread(ThreadPoolExecutor threadPoolExecutor) {
        for (int i = 0; i < 15; i++) {
            int n = i;
            threadPoolExecutor.submit(new Runnable() {
                @Override
    ...
    private void threadPoolExecutorTest8() throws Exception {
        ...
        // 结果分析
        // 1、 10个任务开始执行，3个任务进入队列等待，2个任务被拒绝执行
        // 2、调用shutdownnow后，队列中的3个线程不再执行，10个线程被终止
        // 3、 追加的任务在线程池关闭后，无法再提交，会被拒绝执行
    }

    public static void main(String[] args) throws Exception {
        new ThreadPool().threadPoolExecutorTest1();
//        new ThreadPool().threadPoolExecutorTest2();
//        new ThreadPool().threadPoolExecutorTest3();
//        new ThreadPool().threadPoolExecutorTest4();
//        new ThreadPool().threadPoolExecutorTest5();
//        new ThreadPool().threadPoolExecutorTest6();
//        new ThreadPool().threadPoolExecutorTest7();
//        new ThreadPool().threadPoolExecutorTest8();
    }
}
